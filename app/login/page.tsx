...
function CheckIcon({ met }: { met: boolean }) {
  return (
    <span
      className={`flex items-center justify-center w-4 h-4 rounded border shrink-0 ${met ? 'bg-[#16a34a] border-[#16a34a]' : 'bg-white border-[#E5E7EB] dark:border-[#2A2A2A]'
        }`}
    >
      {met && (
              ...
              <div className="flex justify-center gap-3 mb-6" onPaste={handlePaste}>
                {otp.map((digit, i) => (
                  <input key={i} ref={(el) => { otpRefs.current[i] = el }}
                    ...
                    className={`w-11 h-13 py-2.5 text-center text-lg font-semibold bg-white dark:bg-[#141414] border rounded-lg text-[#0A0A0A] dark:text-white transition-colors focus:outline-none ${digit ? 'border-[#E5002B]' : 'border-[#E5E7EB] dark:border-[#2A2A2A] focus:border-[#E5002B]'
                      }`}
                  />
                ))}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full px-3 py-2.5 pr-14 bg-white dark:bg-[#141414] border rounded-lg text-[#0A0A0A] dark:text-white placeholder-[#9CA3AF] text-[13px] focus:outline-none transition-colors ${confirmPassword.length > 0
